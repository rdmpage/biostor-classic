...
//	$str = preg_replace('/text$/', '', $str);
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/(?<volume_from>\d+)[a-z]{2}-(?<volume_to>\d+)[a-z]{2}\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iu", $str, $m))
		{
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/(?<volume>\d+)[a-z]{2}\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iu", $str, $m))
		{
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/(?<volume>\d+)[a-z]{2}\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^(?<volume>\d+)\s*\((?<issue>\d+)\)$/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/t.(?<volume>\d+)\s*:\s*no.\s*(?<issue>\d+)\s*\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/t.(?<volume>\d+):fasc.(?<issue>\d+)\s*\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/Vol.(?<volume>\d+)\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iu", $str, $m))
		{
			$info->volume = $m['volume'];
			$info->start = $m['year_from'];		
			$info->end = $m['year_to'];	
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/nouv. sér.:t.(?<volume>\d+):fasc.(?<issue>\d+)\s*\((?<year>[0-9]{4})0?\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	// d.13=no.295-318 (1950-1951)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^d\.(?<volume>\d+)=(.*)\s\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iu", $str, $m))
		{
			$info->volume = $m['volume'];
			$info->start = $m['year_from'];		
			$info->end = $m['year_to'];	
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^No\.\s*(?<volume>\d+)\s*\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^No\.(?<volume_from>\d+)-(?<volume_to>\d+)\s*\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iu", $str, $m))
		{
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
			$info->start = $m['year_from'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/b\.(?<volume>\d+)\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/[b|d]\.(?<volume>\d+)\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year_from'];		
			$info->end = $m['year_to'];	
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/[b|d]\.(?<volume_from>\d+)-(?<volume_to>\d+)\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iu", $str, $m))
		{
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
			$info->start = $m['year_from'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/new ser. no.\s*(?<volume>\d+)\s+\((?<year>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	// 
	// <option value="206245">vol.131:no.4(2011:Dec)</option>
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/vol\.(?<volume>\d+):no\.(?<issue>\d+)\((?<year>[0-9]{4}):/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/v(?<volume>\d+):no\.(?<issue>\d+)\s*\((?<year>[0-9]{4})\b/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/(?<volume>\d+)\s+-\s+pt(?<issue>\d+)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue = $m['issue'];		
			$matched = true;
	...
	// Ser.3:tom.8 (1909-1911)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/Ser.(?<series>\d+):tom.(?<volume>\d+)\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
	...
	// Ser.3:tom.9:no.1 (1911)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/Ser.(?<series>\d+):tom.(?<volume>\d+):no\.(?<issue>\d+)\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->series= $m['series'];
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
	...
	// vol.133:no.3(Sep 2013)
	// vol.135:no.3(Sep 2015)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/vol.(?<volume>\d+):no\.(?<issue>\d+)\([A-Z][a-z]+\s+(?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/v,(?<volume>\d+):no(?<issue>\d+)\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^fasc\.(?<volume>\d+)$/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$matched = true;
		}	
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/no\.(?<volume>\d+):pt\.(?<issue>\d+)\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/(?<volume>\d+)\s+no\.?\s*(?<issue>\d+)\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	// new ser. no.61-70 (1978-1980)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/new ser. no.(?<volume_from>\d+)-(?<volume_to>\d+)\s*\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
			$info->start = $m['year_from'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/(?<volume>\d+),\s+no\.?\s*-\s*,\s+(?<year>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/yr\.\s+(?<volume>\d+)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$matched = true;
		}	
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/t\.(?<volume>\d+):\s*pt.(?<issue>.*)\s*\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/v\.(?<volume>\d+),\s*pt.(?<issue>.*)\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/Bd.(?<volume>\d+):Heft\s+(?<issue>\d+)\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/Jahrg.(?<volume>\d+)\s*\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year_from'];		
			$info->end = $m['year_to'];	
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/n.\s*(?<volume>\d+),\s+(?<year>[0-9]{4})/iUu", $str, $m))
		{
			$info->series= $m['series'];
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/v\.\s*(?<volume_from>\d+)-(?<volume_to>\d+)/", $str, $m))
		{
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/ser.\s*(?<series>\d+)\s+v.\s*(?<volume_from>\d+)-(?<volume_to>\d+)$/iUu", $str, $m))
		{
			$info->series= $m['series'];
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^(?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})$/iUu", $str, $m))
		{
			$info->start = $m['year_from'];		
			$info->end = $m['year_to'];	
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/ser.\s*(?<series>\d+):v.(?<volume>\d+)\s*\((?<year>[0-9]{4})/iUu", $str, $m))
		{
			$info->series= $m['series'];
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/ser.\s*(?<series>\d+):v.(?<volume>\d+)\s*\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
			$info->series= $m['series'];
			$info->volume= $m['volume'];
			$info->start = $m['year_from'];		
	...
	// ser.2:no.1-6 (1911-1912)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/ser.\s*(?<series>\d+):no.(?<volume_from>\d+)-(?<volume_to>\d+)\s*\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
			$info->series= $m['series'];
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/ser.\s*(?<series>\d+):no.(?<volume_from>\d+)-(?<volume_to>\d+)\s*\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->series= $m['series'];
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/Ser\.?\s*(?<series>\d+)\.?,?\s+t.\s*(?<volume>\d+)\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->series= $m['series'];
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/fasc.\s*(?<volume>\d+)\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year_from'];		
			$info->end = $m['year_to'];	
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/fasc.\s*(?<volume>\d+)\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/fasc.\s*(?<volume_from>\d+)-(?<volume_to>\d+)\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
			$info->start = $m['year_from'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/fasc.\s*(?<volume_from>\d+)-(?<volume_to>\d+)\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
			$info->start = $m['year_from'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/Vol\.\s+(?<volume>\d+)\((?<issue>\d+)\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$matched = true;
	...
	// n.s.:n.1-15(1898-1899)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/n.s.:n.(?<volume_from>\d+)-(?<volume_to>\d+)\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
			$info->start = $m['year_from'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/v.(?<volume>\d+):pt.(.*)\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year_from'];		
			$info->end = $m['year_to'];	
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/ser\.\s*(?<series>\d+):v\.(?<volume>\d+)/", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->series= $m['series'];
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/pt.(?<volume_from>\d+)-(?<volume_to>\d+)\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
			$info->start = $m['year_from'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/jaard.(?<volume>\d+)\s+(?<year>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/v.(?<volume>\d+):pt\.(?<issue>\d+)\s+\((?<year>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue = $m['issue'];		
			$info->start = $m['year'];		
	...
	// v.27=new ser.:v.22=no.90-93 (1915)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/v.(?<volume>\d+)=(.*)\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/(?<volume>\d+),\s*pt.\s*(?<issue>\d+),\s*(?<year>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	// ser.9:t.5=t.45 (1893)
	// One option, ignore series
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/=t\.(?<volume>\d+)\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	// Jahrg.38-40 (1921-1923)
	// Jahrg.32-34 [Lacks 32:no.12] (1915-1917)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/Jahrg\.(?<volume_from>\d+)-(?<volume_to>\d+)(\s+\[(.*)\])?\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
			$info->start = $m['year_from'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/Jahrg\.?\s*(?<volume>\d+)\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/ptie\.?\s*(?<volume>\d+)\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/vol\.?\s*(?<volume>\d+)(\s+Pt(?<issue>\d+))?\s+\((?<year>[0-9]{4})\)/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/no.(?<volume>\d+)\s+\((?<year>[0-9]{4}):/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	// new ser.:v.22=no.85-88 (1882)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/(new ser.:v.)\s*(?<volume>\d+)=no\.(?<issue>.*)\s+\((?<year>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	// new ser.:v.45=no.177-180 (1901-1902)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/(new ser.:v.)\s*(?<volume>\d+)=no\.(?<issue>.*)\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year_from'];		
			$info->end = $m['year_to'];		
	...
	// v.2-3=no.11-16 (1970-1973)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/v.(?<volume_from>\d+)-(?<volume_to>\d+)=no\.(?<issue>.*)\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
			$info->start = $m['year_from'];		
	...
	// n.s. v.58:no.1-12 (1946:Jan.-Dec.)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/(n.s.|new ser.:)\s*v\.(?<volume>\d+):no\.(?<issue>.*)\s+\((?<year>[0-9]{4}):/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	// n.s. v.40-41:no.1-12 (1928-1929:Jan.-Dec.)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/n.s. v.(?<volume_from>\d+)-(?<volume_to>\d+)\:no\.(?<issue>.*)\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
			$info->start = $m['year_from'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^v.\s+(?<volume_from>\d+)-(?<volume_to>\d+)\s+(?<year>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume_from= $m['volume_from'];
			$info->volume_to= $m['volume_to'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/Jahrg.(?<volume>\d+):no\.(?<issue>\d+[a-z]?)\s*\((?<year>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/Jahrg.(?<volume>\d+):n[o|r]\.(?<issue>\d+[a-z]?)\s*\((?<year>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	// jaarg.42:nr.2(2014:Jun)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/jaarg.(?<volume>\d+):nr\.(?<issue>\d+[a-z]?)\s*\((?<year>[0-9]{4})/iUu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->issue= $m['issue'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/Jarhg.(?<volume>\d+)\((?<year>[0-9]{4})\)$/Uu", $str, $m))
		{
			$info->volume= $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/Jahrg.(?<volume>\d+)\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})\)$/Uu", $str, $m))
		{
			$info->volume= $m['volume'];
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^(?<volume>\d+),$/Uu", $str, $m))
		{
			$info->volume= $m['volume'];
			$matched = true;
		}	
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/Bd.\s*(?<volume>\d+)\s+(?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})$/Uu", $str, $m))
		{
			$info->volume= $m['volume'];
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^Bd.\s+(?<volume>\d+)\s+\((?<year_from>[0-9]{4})\s*-\s*(?<year_to>[0-9]{2})\)$/Uu", $str, $m))
		{
			$info->volume = $m['volume'];
			$info->start = $m['year_from'];		
	...
	// v.42-43;Suppl.6 (2006-2007)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/v.\s*(?<volume_from>\d+)-(?<volume_to>\d+);(.*)\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})\)$/Uu", $str, $m))
		{
			$info->volume_from = $m['volume_from'];
			$info->volume_to = $m['volume_to'];
	...
	// new ser.:v.1:no.1 (1982)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/new ser.:v.(?<volume>\d+):no.(?<issue>\d+)\s+\((?<year_from>[0-9]{4})\)$/Uu", $str, $m))
		{
			$info->volume = $m['volume'];
			$info->issue = $m['issue'];
	...
	// [new ser.:v.7]:pt.1 (1988)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/\[new ser.:v.(?<volume>\d+)\]:pt.(?<issue>\d+)\s+\((?<year_from>[0-9]{4})\)$/Uu", $str, $m))
		{
			$info->volume = $m['volume'];
			$info->issue = $m['issue'];
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/(?<volume>\d+),\s+n.\s+(?<issue>\d+)\s*,\s+(?<year_from>[0-9]{4})$/Uu", $str, $m))
		{
			$info->volume = $m['volume'];
			$info->issue = $m['issue'];
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/(?<volume>\d+\s+-\s+\d+),\s+(?<year_from>[0-9]{4})$/Uu", $str, $m))
		{
			$info->volume = str_replace(' ', '', $m['volume']);
			$info->volume = str_replace('-', '/', $info->volume);
	...
	// new ser.:v.41-44 (1961-1964)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/new ser.:v.\s*(?<volume_from>\d+)-(?<volume_to>\d+)\s+\((?<year_from>[0-9]{4})-(?<year_to>[0-9]{4})\)$/Uu", $str, $m))
		{
			$info->volume_from = $m['volume_from'];
			$info->volume_to = $m['volume_to'];
	...
	// 2e reeks, d.8(1874)-d.9(1876)
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/(?<series>\d+)e reeks,\s+d.(?<volume_from>\d+)\((?<year_from>[0-9]{4})\)-d.(?<volume_to>\d+)\((?<year_to>[0-9]{4})\)$/Uu", $str, $m))
		{
			$info->series = $m['series'];
			$info->volume_from = $m['volume_from'];
			$info->volume_to = $m['volume_to'];
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^(?<volume>20[0-9]{2}-\d+)$/Uu", $str, $m))
		{
			$info->volume = $m['volume'];
			$matched = true;
		}	
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^Heft\s+(?<volume>\d+)\s*\((?<year>[0-9]{4}):\s*\w+\.?\)/Uu", $str, $m))
		{
			$info->volume = $m['volume'];
			$info->start = $m['year'];		
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^Heft\s+(?<volume_from>\d+)\s*-\s*(?<volume_to>\d+)\s+\((?<year>[0-9]{4}):\s*\w+\.?\)/Uu", $str, $m))
		{
			$info->volume_from = $m['volume_from'];
			$info->volume_to = $m['volume_to'];
			$info->start = $m['year'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^(?<volume>\d+)\s+abt.\s+(?<issue>\d+(-\d+)?)/Uu", $str, $m))
		{
			$info->volume 	= $m['volume'];
			$info->issue 	= $m['issue'];
			$matched = true;
	...
	// v.37 abt.2 1919-1920
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^v\.(?<volume>\d+)\s+abt.(?<issue>\d+)\s+(?<year_from>[0-9]{4})\s*-\s*(?<year_to>[0-9]{4})$/Uu", $str, $m))
		{	
			$info->volume = $m['volume'];
			$info->issue = $m['issue'];
			$info->start = $m['year_from'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^ser\.(?<series>\d+):v\.(?<volume>\d+)\s+\((?<year_from>[0-9]{4})\s*-\s*(?<year_to>[0-9]{4})\)$/Uu", $str, $m))
		{
			$info->series = $m['series'];
			$info->volume = $m['volume'];
			$info->start = $m['year_from'];		
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^t\.\s*(?<volume_from>\d+)\s*-\s*(?<volume_to>\d+)\s+\((?<year_from>[0-9]{4})\s*-\s*(?<year_to>[0-9]{4})\)$/Uu", $str, $m))
		{
			$info->volume_from = $m['volume_from'];
			$info->volume_to = $m['volume_to'];
			$info->start = $m['year_from'];		
	...
	// t.1-2 (1876) [Lacks:Pg.55-56]
	// t.127-128 (1939) [Incomplete]
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^t\.(?<volume_from>\d+)-(?<volume_to>\d+)\s+\((?<year_from>[0-9]{4})\)\s+\[/Uu", $str, $m))
		{
			$info->volume_from = $m['volume_from'];
			$info->volume_to = $m['volume_to'];
			$info->start = $m['year_from'];		
	...
	// t.6 (1878) [Lacks:Pg.49-96]
	if (!$matched)
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^t\.(?<volume>\d+)\s+\((?<year_from>[0-9]{4})\)\s+\[/Uu", $str, $m))
		{
			$info->volume = $m['volume'];
			$info->start = $m['year_from'];		
			$matched = true;
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^Bd.(?<volume_from>\d+)\/(?<volume_to>\d+)\s+\((?<year>[0-9]{4})\)$/Uu", $str, $m))
		{
			$info->volume_from = $m['volume_from'];
			$info->volume_to = $m['volume_to'];
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^t\.(?<volume>\d+)\s*\(\w+\s+(?<year>[0-9]{4})\)/Uu", $str, $m))
		{
			$info->volume 	= $m['volume'];
			$info->issue 	= $m['issue'];
			$info->start 	= $m['year'];	
	...
	{
		if ($debug) echo "Trying " . __LINE__ . "\n";
		if (preg_match("/^new ser. v. (?<volume>\d+)\s+\((?<year_from>[0-9]{4})\s*-\s*(?<year_to>[0-9]{2})\)$/Uu", $str, $m))
		{
			$info->volume = $m['volume'];
			$info->start = $m['year_from'];		
			$info->end = substr ($m['year_from'], 0, 2) . $m['year_to'];	
